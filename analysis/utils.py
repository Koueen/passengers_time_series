...
def plot_forecasting(passengers: pd.Series, model_fitted: pd.Series, predictions: pd.Series, title: str, is_log: bool):
    fig = go.Figure()
    ...
    fig.update_layout(
         yaxis=dict(
            title=y_title,
            range=range),  # Set y-axis limits (ylim) from 0 to 80
        title=title,
        title_x=0.5,
        title_y= 0.90,
        legend=dict(
...
def get_metrics(model_fit, predictions, train_set, test_set):
    ...
    print(pd.DataFrame({'Model' : ['Holt-Winters','Holt-Winters'],
